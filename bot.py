...
def get_docx_content(file_token):
    ...
    try:
        ...
        print(f"[读取文档异常] {str(e)}")
        raise e

def send_reply(message_id, content, msg_type="text"):
    if not message_id:
        print("[send_reply] message_id 为空，跳过发送")
        return None
    try:
        token = get_tenant_access_token()
        ...
        res = resp.json()
        print(f"[发送文本回复] msg_id={message_id}, 返回={res}")
        return res
    except Exception as e:
        print(f"[发送文本回复异常] {str(e)}")
        return None

def send_file_message(message_id, file_token, file_name):
    if not message_id:
        print("[send_file_message] message_id 为空，跳过发送")
        return None
    try:
        token = get_tenant_access_token()
        url = f"{FEISHU_API_BASE}/im/v1/messages/{message_id}/reply"
        ...
        payload = {
            "msg_type": "file",
            "content": json.dumps({"file_token": file_token, "file_name": file_name})
        }
        resp = requests.post(url, json=payload, headers=headers, timeout=30)
        res = resp.json()
        print(f"[发送文件回复] msg_id={message_id}, 返回={res}")
        return res
    except Exception as e:
        print(f"[发送文件回复异常] {str(e)}")
        return None
...
def handle_message(event):
    try:
        ...
        print(f"[消息基础信息] message_id={message_id}")

        if not message_id:
            print("[警告] message_id 获取为空，终止处理")
            return

        content_str = msg_info.get("content", "{}")
        content = json.loads(content_str)
        msg_type = content.get("msg_type", "")
        text = content.get("text", "")
        print(f"[用户消息] msg_type={msg_type}, text={text}")
        ...
        if msg_type == "text":
            ...
            if doc_match:
                ...
                if df.empty:
                    ...
                    return
                send_excel_result(message_id, df, f"知识清单_{datetime.now().strftime('%Y%m%d')}")
            else:
                # 普通文字无文档链接，回复提示
        ...
        print(f"[handle_message全局异常] {str(e)}")
        return

# 异步后台任务包装
def async_task_wrapper(event):
    try:
        handle_message(event)
    except Exception as e:
        print(f"[异步任务崩溃] {str(e)}")
...
def webhook():
    ...
    if data.get("type") == "event_callback":
        ...
        if event_type == "im.message.receive_v1":
            # 新开线程异步处理，立刻返回，不阻塞3秒限制
            t = threading.Thread(target=async_task_wrapper, args=(event,))
            t.start()
